...
import 'package:austernotes/services/auth/auth_provider.dart';
    ...
    test('Create user should delegate to logIn function', () async {
      ...
      expect(
          ...
          throwsA(const TypeMatcher<
              UserNotFoundAuthException>())); // The specific email used should cause an Exception
...
class MockAuthProvider implements AuthProvider {
  ...
  Future<AuthUser> createUser({
    ...
    await Future.delayed(const Duration(
        seconds: 1)); // Mock the delay of connecting to the backend
    return logIn(
      email: email,
      password: password,
    );
  ...
  Future<void> initialize() async {
    await Future.delayed(const Duration(
        seconds: 1)); // Mock the delay of initializing the module
    _isInitialized = true;
  }
  ...
  Future<void> logOut() async {
    ...
    await Future.delayed(
        const Duration(seconds: 1)); // Mock the delay of logging out
    _user = null; // Logged out
  }
